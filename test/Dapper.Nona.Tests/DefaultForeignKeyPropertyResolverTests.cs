...
namespace Dapper.Nona.Tests
{
    public class DefaultForeignKeyPropertyResolverTests
    {
        ...
        public void Resolves_OneToMany_WithDefaultConvetions()
        {
            ...
            // Act
            var fk = resolver.Resolve(typeof(Product), typeof(ProductOption), out var fkRelation);

            // Assert
            Assert.Equal(typeof(ProductOption).GetProperty(nameof(ProductOption.ProductId)), fk.PropertyInfo);
        ...
        public void Resolves_OneToOne_WithAttributes()
        {
            ...
            // Act
            var fk = resolver.Resolve(typeof(ProductDto), typeof(CategoryDto), out var fkRelation);

            // Assert
            Assert.Equal(typeof(ProductDto).GetProperty(nameof(ProductDto.CategoryId)), fk.PropertyInfo);
            Assert.Equal(ForeignKeyRelation.OneToOne, fkRelation);
        }
